...
pub fn adjacent_indexes(input: &String, index: usize, include_diagonals: bool) -> Vec<usize> {
    ...
    let index_less_one = if index.checked_sub(1).is_some_and(|i| !is_newline(input, i)) {
        Some(index - 1)
    } else {
        None
    };
    let index_plus_one = if is_newline(input, index + 1) {
        None
    } else {
        Some(index + 1)
    };

    if let Some(i) = index.checked_sub(line_len) {
        indexes.push(i)
    }
    if is_in_bounds(input, index + line_len) {
        indexes.push(index + line_len)
    }

    if let Some(i) = index_less_one {
        indexes.push(i);
        if include_diagonals {
            if let Some(i_less_row) = i.checked_sub(line_len) {
                indexes.push(i_less_row)
            }
            if is_in_bounds(input, i + line_len) {
                indexes.push(i + line_len)
            }
        }
    }
